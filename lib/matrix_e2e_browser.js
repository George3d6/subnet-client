...
class E2EMatrixClient {
  ...
  async login(username, password, opts = {}) {
    ...
    // Cross-signing setup. Two cases matter:
    //   (a) Server has no master key yet → bootstrap a fresh identity.
    //   (b) Server already has a master (another device of ours set it up)
    //       and we don't hold the secrets → DO NOT re-bootstrap (would
    //       clobber the trust web). Surface 'awaiting_verification'
    //       instead; verification from a trusted device will gossip the
    //       secrets in.
    try {
      ...
      if (crypto) {
        ...
        if (!hasServerKeys) {
          ...
          this._crossSigningStatus = { state: 'awaiting_verification' };
          console.warn(
            '[E2E] cross-signing: server already has identity from another device. ' +
            'Verify this session from a trusted device (Settings → Sessions).',
          );
        }
      }
    } catch (e) {
