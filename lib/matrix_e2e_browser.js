...
class E2EMatrixClient {
   ...
   *   for accountability signing in place of the main `privateKey`/`signer`.
   *   When set alongside `delegation`, this is the delegate signer.
   * @param {object} [opts.delegation] - Optional delegation envelope to
   *   attach to outbound message accountability fields.
   */
  constructor({ matrixUrl, privateKey, signer, userAddress, storePath, messageSigner, delegation }) {
  ...
  _signingHandle() {
    if (this.messageSigner) return this.messageSigner;
  ...
  async sendMessage(roomId, text, opts = {}) {
    ...
    const signed = await signMessage(this._signingHandle(), historyEvents, text, this.userId, this.delegation);
