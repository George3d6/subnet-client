...
class E2EMatrixClient {
   ...
   * @param {object} [opts.messageSigner] - Optional override signer used
   *   for accountability signing in place of the wallet built from
   *   `privateKey`. When passed alongside a `delegation` envelope, this
   *   should be the delegate's signer.
   * @param {object} [opts.delegation] - Optional delegation envelope to
   *   attach to outbound message accountability fields.
   */
  constructor({ matrixUrl, privateKey, storePath, messageSigner, delegation }) {
    this.matrixUrl = matrixUrl.replace(/\/$/, '');
    this.privateKey = privateKey;
    this.messageSigner = messageSigner || null;
  ...
  async sendMessage(roomId, text, opts = {}) {
    ...
    const signingHandle = this.messageSigner || this.privateKey;
    const signed = await signMessage(signingHandle, historyEvents, text, this.userId, this.delegation);
