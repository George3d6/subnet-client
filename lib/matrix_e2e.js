...
class E2EMatrixClient {
  ...
  async _projectEvent(event, encrypted, displayNames, roomId) {
    ...
    if (FILE_TYPES.includes(msgtype)) {
      ...
      return {
        ...
        timestamp: event.origin_server_ts,
        attachment: { msgtype, mxc_url: mxcUrl, filename, mimetype, encrypted: isEncrypted },
      };
    }
   ...
   * @param {boolean} [opts.advanceCheckpoint=true] - Whether to persist the
   *   new checkpoint after reading. Set to false for a peek that doesn't
