...
class E2EMatrixClient {
  ...
  async readAllNewMessages(opts = {}) {
    ...
    const token = memory.getTimelineSyncToken && memory.getTimelineSyncToken();

    if (token) {
      try {
        return await this._readAllNewMessagesDelta(opts, token, OLD_CONTEXT_COUNT);
      } catch (e) {
        // Stale or unknown token: drop it and fall through to a fresh
        // bootstrap via the per-room /messages path. Anything other than a
        // 4xx propagates — we want callers to see real errors.
        if (e?.status >= 400 && e?.status < 500) {
          memory.clearTimelineSyncToken && memory.clearTimelineSyncToken();
        } else {
          throw e;
        }
      }
    ...
    // Bootstrap (or post-fallback): use the legacy per-room /messages path
    // and stamp a fresh next_batch so the next call can take the delta path.
    const result = await this._readAllNewMessagesLegacy(opts, OLD_CONTEXT_COUNT);
    const markAsRead = opts.markAsRead ?? opts.mark_as_read ?? opts.advanceCheckpoint;
    if (markAsRead !== false) {
      try {
        const next = await this._stampTimelineToken();
        if (next && memory.setTimelineSyncToken) memory.setTimelineSyncToken(next);
      } catch {
        // Best-effort: a failed token stamp just means the next call falls
        // back to the legacy path again. Not fatal.
      }
    }
    return result;
  }
  ...
  /**
   * Issue a `timeline.limit=0` /sync purely to obtain a current next_batch
   * token. Used after a bootstrap walk so subsequent readAllNewMessages
   * calls can take the delta path. Does NOT touch the to-device cursor
   * (the to-device file is owned by `_syncOnce`).
   */
  async _stampTimelineToken() {
    const filter = JSON.stringify({
      room: { timeline: { limit: 0 }, ephemeral: { limit: 0 }, state: { lazy_load_members: true } },
      presence: { limit: 0 },
    });
    const params = new URLSearchParams({ timeout: '0', filter });
    const sync = await this._fetch(`/_matrix/client/v3/sync?${params}`);
    return sync.next_batch || null;
  }

  /**
   * Delta path. Issues a single /sync since the stored timeline token,
   * processes per-room timeline events into the same shape the legacy
   * path produces, and tops up `old_context` from a per-room snapshot
   * cache that the legacy path seeded on bootstrap.
   */
  async _readAllNewMessagesDelta(opts, token, oldContextCount) {
    const markAsRead = opts.markAsRead ?? opts.mark_as_read ?? opts.advanceCheckpoint;
    ...
    const params = new URLSearchParams({ timeout: '0', since: token, filter });
    const sync = await this._fetch(`/_matrix/client/v3/sync?${params}`);
    ...
    for (const roomId of roomIds) {
      try {
        ...
        const timeline = join.timeline?.events || [];

        // Project each timeline event into our message / status shape.
        // Reuse the same projection helpers the legacy path uses so the
        // returned objects are byte-compatible.
        ...
        // Pull cached old_context from the snapshot, then fold the new
        // events into the snapshot so the next call still has 10 items
        // even if the room goes quiet.
        const snapshot = memory.getRoomSnapshot
          ? memory.getRoomSnapshot(roomId)
          : { messages: [], status: [] };
        const oldContext = (snapshot.messages || []).slice(-oldContextCount);
        const oldStatusContext = (snapshot.status || []).slice(-oldContextCount);

        if (advanceCheckpoint && (newMessages.length || statusEvents.length)) {
          const mergedMsgs = [...(snapshot.messages || []), ...newMessages].slice(-oldContextCount);
          const mergedStatus = [...(snapshot.status || []), ...statusEvents].slice(-oldContextCount);
          if (memory.setRoomSnapshot) memory.setRoomSnapshot(roomId, { messages: mergedMsgs, status: mergedStatus });
        }

        // Advance the per-room timestamp checkpoint for back-compat with
        // any caller still reading checkpoint_ms (e.g. has_new probes).
    ...
    if (advanceCheckpoint && sync.next_batch) {
      memory.setTimelineSyncToken && memory.setTimelineSyncToken(sync.next_batch);
    }
  ...
  /**
   * Original per-room /messages?dir=b path. Kept intact as the bootstrap
   * and 4xx-fallback for the delta path. Also seeds the per-room snapshot
   * cache that the delta path consumes for `old_context`.
   */
  async _readAllNewMessagesLegacy(opts = {}, OLD_CONTEXT_COUNT = 10) {
    ...
    for (const roomId of roomIds) {
      try {
        ...
        // Advance the checkpoint past the newest event we surfaced — message
        // OR status. Otherwise a flurry of joins after the last text would
        // re-surface on every read.
        const newestTs = Math.max(
          newMessages.length ? newMessages[newMessages.length - 1].timestamp : 0,
          statusEvents.length ? statusEvents[statusEvents.length - 1].timestamp : 0,
        );
        ...
        // Seed the snapshot cache so the delta path has old_context ready
        // on the next call. Combine pre-cutoff (old_context) and the just-
        // surfaced new messages, keep the most recent OLD_CONTEXT_COUNT.
        if (advanceCheckpoint && memory.setRoomSnapshot) {
          const snapMsgs = [...(oldContext || []), ...(newMessages || [])].slice(-OLD_CONTEXT_COUNT);
          const snapStatus = [...(oldStatusContext || []), ...(statusEvents || [])].slice(-OLD_CONTEXT_COUNT);
          memory.setRoomSnapshot(roomId, { messages: snapMsgs, status: snapStatus });
        }
  ...
  async getDirects() {
    ...
    try {
      ...
      if (e && /404/.test(String(e.message || e))) return {};
      throw e;
    }
  }
  ...
  async _getMyMembership(roomId) {
    ...
    try {
      ...
      if (e && /404/.test(String(e.message || e))) return null;
      throw e;
    }
  }
