...
class E2EMatrixClient {
  ...
  async _ensureCrossSigning(password) {
    ...
    } else if (serverMasterEd25519) {
      // Multi-device case: server already has a cross-signing identity
      // belonging to this user (set up by another of their devices). We
      // must NOT clobber it — that would invalidate every device the
      // user has already verified. Wait until another already-trusted
      // device verifies us and gossips the master/SSK/USK private bytes
      // via m.secret.send (see receiveSecretSend in this file).
      this._crossSigningStatus = {
        state: 'awaiting_verification',
        serverMasterKey: serverMasterEd25519,
      };
      console.warn(
        `[E2E] cross-signing: server already has identity ${serverMasterEd25519} ` +
        `from another of this user's devices. This device will appear unverified ` +
        `until verified from one of those devices (e.g. agora "Verify this session").`,
      );
      return;
    } else {
      // Greenfield: server has nothing. Mint a fresh identity and upload.
