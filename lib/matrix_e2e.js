...
class E2EMatrixClient {
   ...
   * @param {string} [opts.storePath] - Directory for session + crypto state (default: .matrix-session)
   */
  constructor({ matrixUrl, privateKey, storePath }) {
    this.matrixUrl = matrixUrl.replace(/\/$/, '');
    this.privateKey = privateKey;
    this.storePath = path.resolve(storePath || '.matrix-session');
  ...
  async readMessages(roomId, opts = {}) {
    ...
    if (opts.from) reqPath += `&from=${encodeURIComponent(opts.from)}`;
    ...
    return { messages: result, end: data.end };
  }
