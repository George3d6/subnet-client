...
class SubnetClient {
   ...
   * @param {true|{from?: number, until?: number, force?: boolean, storePath?: string, namespace?: string}} [opts.delegation]
   ...
   *   Pass `true` for defaults; pass an object to override:
   *     - `from`/`until` — UNIX seconds bounding validity
   *     - `force` — ignore the cached envelope and mint a fresh one
   *     - `storePath` / `namespace` — override storage location
   */
  constructor({ privateKey, signer, address, apiBase, signMessage, delegation }) {
    ...
    if (delegation) {
      ...
      this._delegationOpts = {
        from: o.from ?? null,
        until: o.until ?? null,
        force: o.force === true,
        storePath: o.storePath || null,
  ...
  /**
   * Idempotently resolve a usable delegation. Loads a cached envelope if
   * still valid; otherwise generates a fresh delegate wallet and signs a
   * new envelope with the main signer (one MetaMask popup).
   */
  async _ensureDelegation() {
    ...
    this._delegationReady = (async () => {
      ...
      // Namespace browser caches by main address so multiple accounts in
      // the same origin don't trample each other's delegation key.
      const namespace = this._delegationOpts.namespace ||
        `subnet-client-state:${String(this.address).toLowerCase()}`;
      const store = createDelegationStore({
        storePath: this._delegationOpts.storePath,
        namespace,
      });
      if (!this._delegationOpts.force) {
        const cached = await store.load(this.address);
        if (
          cached &&
          cached.envelope &&
          cached.delegate?.privateKey &&
          isDelegationActive(cached.envelope) &&
          String(cached.envelope.to).toLowerCase() === String(cached.delegate.address).toLowerCase() &&
          String(cached.envelope.delegator).toLowerCase() === String(this.address).toLowerCase()
        ) {
      ...
      if (String(envelope.delegator).toLowerCase() !== String(this.address).toLowerCase()) {
        throw new Error(
          `Delegation signer mismatch — envelope signed by ${envelope.delegator}, expected ${this.address}`,
        );
      }
      await store.save({
        envelope,
  ...
  async loginMatrix(opts = {}) {
    ...
    this.matrix = new E2EMatrixClient({
      ...
      messageSigner: this._delegateWallet || null,
