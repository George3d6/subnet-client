...
class SubnetClient {
  ...
  /**
   * Fetch the current user's metadata, ABT balance, and address from the subnet.
   * @returns {Promise<{address: string, metadata: string, abt: number}>}
   */
  async getMetadata() {
    const signature = await this._sign();
  ...
  /**
   * Cast a yes/no vote on a gated action. Handles the EIP-191 signature.
   * @param {string} uuid
   * @param {'yes'|'no'|'y'|'n'|boolean} vote
   */
  async castVote(uuid, vote) {
    if (!uuid) throw new Error('uuid is required');
    const normalized = typeof vote === 'boolean'
      ? (vote ? 'yes' : 'no')
      : String(vote).toLowerCase();
    let yes;
    if (normalized === 'yes' || normalized === 'y') yes = true;
    else if (normalized === 'no' || normalized === 'n') yes = false;
    else throw new Error(`vote must be yes/no (got "${vote}")`);
    const message = `Vote ${yes ? 'Yes' : 'No'} ${uuid}`;
  ...
  /**
   * Move liquid ABT into a time-locked stake behind an address (self or
   * another member). Governance weight (VVM voting, slashing) reads staked
   * balances — unstaked ABT has no weight.
   *
   * @param {string} stakedBehind - Address the stake backs (self-stake or delegated).
   * @param {number|string} amount - ABT to move from liquid balance into the stake.
  ...
  async createStake(stakedBehind, amount, durationSeconds = 86400, release = false) {
    ...
    const message = `Stake ${amtStr} ABT behind ${behindLower} for ${duration}s`;
    const signature = await this.wallet.signMessage(message);
