...
class SubnetClient {
   ...
   *   Defaults to MATRIX_SESSION_PATH env var or '.matrix-session' in the current directory.
   *   Pass opts.e2e = false to use the legacy non-E2E client (not recommended).
   */
  async loginMatrix(opts = {}) {
    ...
    if (opts.e2e === false) {
      ...
      const storePath = opts.storePath || process.env.MATRIX_SESSION_PATH || '.matrix-session';
