...
class MemoryStore {
  ...
  constructor(storePath) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS timeline_sync (
  ...
  // ── Timeline /sync cursor (separate from to-device sync_token file) ───────
  //
  // readAllNewMessages walks a /sync stream that includes room timelines.
  // We persist its next_batch separately from the to-device sync_token kept
  // by _syncOnce so that a send-then-read sequence (which calls _syncOnce
  // to drain to-device) doesn't accidentally advance past unread timeline
  // events.

  getTimelineSyncToken() {
    const row = this.db.prepare('SELECT next_batch FROM timeline_sync WHERE id = 1').get();
    return row ? row.next_batch : null;
  }

  setTimelineSyncToken(token) {
    if (!token) return;
    this.db
      .prepare(
        `INSERT INTO timeline_sync (id, next_batch, updated_at)
         VALUES (1, ?, ?)
  ...
  clearTimelineSyncToken() {
    this.db.prepare('DELETE FROM timeline_sync WHERE id = 1').run();
  }

  // ── Room "old_context" snapshots ──────────────────────────────────────────
  //
  // The /sync delta path only returns events newer than the cursor, so we
  // can't satisfy old_context (the last ~10 messages from before the
  // cutoff) from the delta alone. Cache the most recent surfaced messages
  // and membership events per room; bootstrap fills it via /messages once.

  getRoomSnapshot(roomId) {
    const row = this.db
      .prepare('SELECT recent_json, status_json FROM room_snapshots WHERE room_id = ?')
      .get(roomId);
    if (!row) return { messages: [], status: [] };
    try {
      return {
        messages: JSON.parse(row.recent_json) || [],
  ...
  setRoomSnapshot(roomId, snapshot) {
    ...
    const status = JSON.stringify(snapshot.status || []);
    this.db
      .prepare(
        `INSERT INTO room_snapshots (room_id, recent_json, status_json, updated_at)
         VALUES (?, ?, ?, ?)
      ...
      .run(roomId, messages, status, Date.now());
  }
