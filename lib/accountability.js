 ...
 */
async function signMessage(signerOrKey, history, message, sender) {
  const signer =
...
function parseConversation(text) {
  ...
  return blocks.filter(b => b.trim()).map(block => {
    ...
    const fields = { with_reply: null, prev_conv: null, reply_only: null };
    ...
    const { with_reply, prev_conv, reply_only } = fields;
